...
async function run() {
  try {
    console.log('Fetching Consumer_Master data...');
    const response = await sheetsClient.spreadsheets.values.get({
      spreadsheetId,
      range: `'Consumer_Master'`,
    });
    const rows = response.data.values || [];
    if (rows.length < 2) {
      console.log('No data found in Consumer_Master.');
      return;
    }
    ...
    cids.forEach(cid => {
      const matchIdx = rows.findIndex(r => String(r[0] || '').trim() === cid);
      if (matchIdx !== -1) {
        ...
        console.log(`\nConsumer ID: ${cid} not found in Consumer_Master.`);
      }
    });
  } catch (err) {
